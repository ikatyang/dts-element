...
import {create_curried_fruntion_interfaces} from '../create-curried-function-interfaces';

it('should return correctly without placeholder', () => {
  const interfaces = create_curried_fruntion_interfaces({max_curry_level: 3});
...
it('should return correctly with placeholder', () => {
  ...
  const interfaces = create_curried_fruntion_interfaces({max_curry_level: 3, placeholder});
  const document = new Document({children: interfaces});
  expect(document.emit()).toMatchSnapshot();
});
