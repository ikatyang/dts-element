...
it('should return emitted type while flag is not rest', () => {
  expect(emit_parameter_type(any_type, ParameterFlag.REST, new Stack())).toMatchSnapshot();
});

it('should return emitted type wrapped with ArrayType while flag is rest', () => {
  expect(emit_parameter_type(any_type, ParameterFlag.NONE, new Stack())).toMatchSnapshot();
});
