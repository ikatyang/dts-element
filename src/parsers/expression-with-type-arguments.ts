...
export const parse_expression_with_type_arguments = (node: ts.ExpressionWithTypeArguments): IGeneralType => {
  const generics = if_defined(node.typeArguments, type_args => type_args.map(parse_native));
  if (node.expression.kind === ts.SyntaxKind.Identifier) {
    ...
    const names = parse_property_access_expression(node.expression as ts.PropertyAccessExpression);
    return create_general_type({
