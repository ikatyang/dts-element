...
export interface ITypePredicate
  extends IElement<ElementKind.TypePredicate>,
    ITypePredicateOptions {}

export const create_type_predicate = (
  options: ITypePredicateOptions,
): ITypePredicate => ({
 ...
 * @hidden
 */
export const transform_type_predicate = (
  element: ITypePredicate,
  path: IElement<any>[],
) =>
    ...
    /* parameterName */ typeof element.parameter === 'string'
      ...
      : (() => {
          if (element.parameter.type.kind !== ts.SyntaxKind.ThisKeyword) {
            throw new Error(
              `type_predicate.parameter should be a string or this_type`,
            );
          }
