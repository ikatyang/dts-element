...
export interface IInterfaceDeclaration
  extends IElement<ElementKind.InterfaceDeclaration>,
    IInterfaceDeclarationOptions {}

export const create_interface_declaration = (
  options: IInterfaceDeclarationOptions,
 ...
 * @hidden
 */
export const transform_interface_declaration = (
  element: IInterfaceDeclaration,
  path: IElement<any>[],
) =>
  ts.createInterfaceDeclaration(
    ...
    /* heritageClauses */ element.extends && [
      ts.createHeritageClause(
        /* token */ ts.SyntaxKind.ExtendsKeyword,
        /* types */ element.extends.map(extend =>
          create_expression_for_general_type(extend, path),
        ),
      ),
    ],
    /* members         */ (transform(
      element.type || create_object_type(),
      path,
    ) as ts.TypeLiteralNode).members,
  );
